...
\begin{figure}[h!]
  ...
  \caption{Scatterplot of the xenon uptake as a function of the selectivity and labeled by the values of the LCD. The selectivities and uptakes are calculated by a GCMC simulation of a 20:80 Xe/Kr mixture. The most selective materials are colored in orange-red or yellow corresponding to a pore size adapted for xenon adsorption. The less selective ones are in red, with a pore lower than the size of a xenon hence preventing its adsorption. \todo{change color ?}}\label{fgr:vol}
\end{figure}
...
%higher uptakes
We will now see the effect of the composition on the different analysis we carried out on the different structural descriptors. One of the major changes when considering a mixture with much more xenon, is the values of the xenon uptake. The nanopores of selective materials ($s>1$) are much more saturated in Xe and the maximum amount of xenon is therefore much more higher. By comparing the Figures~\ref{fgr:vol} and \ref{fgr:compo}, the maximum uptake is now \SI{12}{\milli\mole\per\gram} \todo{exact values} instead of \SI{7}{\milli\mole\per\gram} (for the 20:80 composition). We previously stated that the most selective materials could reach up to \SI{4}{\milli\mole\per\gram} of xenon uptake, for a composition with a higher xenon content, it can now reach up to \SI{6}{\milli\mole\per\gram}. These results are consistent with materials that are not yet saturated and will take more xenon as the proportion of xenon in the mixture increases.
