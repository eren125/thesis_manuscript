...
The MD trajectory can be used to generate displacement of any size $\tau$. Naively, we can take $\lVert\mathbf{r}_i(\tau)-\mathbf{r}_i(0)\rVert$ as a displacement in a time frame $\tau$. However, it is not enough to make a statistically meaningful average as described in the Einstein equation\ref{eq:einstein}. Using the hypothesis of independence between two movements of the same particle separated by a time $\delta t$ that Einstein used in his derivation, a shift of the origin of time by $\delta t$ would generate another trajectory. We can therefore generate $(t\e{tot} -\tau)/ \delta t$ different time frames of duration $\tau$ with a single MD simulation of one particle.
...
Before going into the details of the screening study, we will present the approach of diffusion coefficient calculation using MSD values on the example of SBMOF-1\autocite{Banerjee_2016}. This preliminary study will help us calibrate the time parameters (time step, maximum time) that will be finally used in the screening study of the most selective materials. 

\todo{Pictures of fittings}


If we don't have enough time steps > don't reach the diffusion regime
