...
Now that we have well-defined channels and pockets of our nanoporous material, we can identify the symmetrically equivalent channels by using the symmetry of grid as already explained in the section~\ref{sct:grid}. We now end with usually a few unique channels (less than three in most cases) that can be used for the basin-cluster growth and the detection of TS surfaces that separate these basins. 

\todo{explain mergin point detection with large energy steps}

Make the layer by layer growth only if there is a change in the number of clusters (a merge occured) > skip first steps. At the first merge we need to pass in layer-b-layer mode. 

The planned next steps for the detection of TS (not done yet) are: (i) defining the bassin grid points, (ii) define a layer by layer cluster growth, and (iii) define the boundary point as another entity that will give the surface. 
