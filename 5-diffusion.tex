...
When considering the ambient-pressure selectivity, the large majority of highly selective materials actually have a rather low diffusion selectivity (lower than $0.1$), as we can see on the Figure~\ref{fgr:perm_selec2080} (this was not the case for the low-pressure selectivity). This result actually suggest that we actually need to make a trade-off between adsoprtion selectivity and diffusion selectivity. In our approach, we decided to lower the adsorption diffusivity to values around $40$ to allow for higher diffusion selectivity values, because the approach up until now only mazimized the adsorption selectivity and the values on the lower right side of the plots of Figure~\ref{fgr:perm_selec2080} are actually already obtained and studied in standard screenings in the literature\autocite{Simon_2015,Chung_2019} but also in one of my published work~\ref{Ren_2021}. Also, we did not try to optimize the perm-selectivity also known as the membrane selectivity because it is specifically useful for membrane separations and many studies in the literature focus on this aspect of the screening\autocite{Anderson_2017,Wang_2022}. From the screening we presented here, we rather want to find thermodynamically selective materials that are not limited by diffusion and identified some interesting materials that will be further studied in the following subsection. 


\subsubsection{Identification of interesting materials}

\todo{Visualization of the energy map inside}
...
\hline
ADOGEH\autocite{Peikert_2012} & 12.9 &  10 & 49 & 5.3 & 15.5 &               5$\times$10\ex{-5} \si{\square\centi\meter\per\second} \\
XUNSOQ\autocite{Abrahams_2014} &  5.6 & 48 & 38 & 4.8 &  0.23 &  7$\times$10\ex{-6}~\si{\square\centi\meter\per\second} \\
GUMDEZ\autocite{Yin_2014} &  5.5 & 42 & 56 & 5.1 &  0.55 &               7$\times$10\ex{-5} \si{\square\centi\meter\per\second} \\
QOZDOY\autocite{Zhang_2001} &  5.6 & 37 & 52 & 5.0 &  0.45 &               7$\times$10\ex{-5} \si{\square\centi\meter\per\second} \\
MISQIQ\autocite{Tong_2013} &  4.6 & 37 & 140 & 4.5 &  1.4 &               2$\times$10\ex{-4} \si{\square\centi\meter\per\second} \\
\hline
\end{tabular}
