...
\begin{table}[ht]
  ...
  \begin{tabular}{|l|r|r|}
    ...
    \hline
  \end{tabular}
  \caption{Performance comparison of the new grid method to other standard techniques used to calculate the xenon adsorption enthalpies. The RMSE is calculated by comparing to the values given by a 100k-steps Widom insertion considered as the ground truth. The associated calculations are performed on the structures with an LCD\e{CCDC} over \SI{3.7}{\angstrom} CoRE MOF 2019 database with a single Intel Xeon Platinum 8168 core at 2.7~GHz. \todo{incohérence sur les rmse et sur la definiton du LCD\e{CCDC}, refaire/}}\label{tab:grid}
\end{table}
